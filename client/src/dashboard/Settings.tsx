...
/**
 * Settings: the user's profile and security. Email and role are shown but not editable here (role is
 * an operator action; changing email would need re-verification, currently disabled). Editable:
 * display name and password. 2FA and KYC are stubbed as the next security steps.
 */
      ...
      <section className="acct-card acct-profile">
        ...
        <div className="acct-id">
          ...
          <div className="acct-badges">
            ...
            <span className={`acct-badge ${user.is_verified ? "ok" : "muted"}`}>{user.is_verified ? "Verified" : "Unverified"}</span>
          </div>
        </div>
...
function ProfileForm() {
  ...
  async function save() {
    ...
    try { const u = await api.updateProfile({ full_name: name.trim() }); setUser(u); setNote("Profile updated"); }
  ...
  return (
    <section className="acct-card">
      <h2>Profile</h2>
      <label className="acct-field"><span>Display name</span>
...
function PasswordForm() {
  ...
  async function save() {
    ...
    try {
      ...
      setNote("Password updated"); setCur(""); setNext(""); setConfirm("");
    } catch (e) { setErr(e instanceof ApiError ? e.message : String(e)); }
    finally { setBusy(false); }
  }
