...
export function FuturesTrade() {
  ...
  return (
    <div className="trade">
      {xfer && <TransferModal onClose={() => setXfer(false)} onDone={(a) => { setAcct(a); setXfer(false); }} />}
      <div className="term-grid mgt">
        <div className="g-ticker tk-bar">
          ...
          <div className="mgt-ml"><span>Futures Balance</span><b>{acct ? `${Number(acct.balance_usdt).toFixed(2)} USDT` : "—"}</b></div>
        ...
        <section className="g-form tp">
          {kycOk === false ? <KycRequiredNotice /> :
            <FuturesForm symbol={symbol} balance={Number(acct?.balance_usdt ?? 0)} livePrice={ticker?.price ?? null} onDone={loadAcct} />}
        </section>
...
function FuturesForm({ symbol, balance, livePrice, onDone }: { symbol: string; balance: number; livePrice: number | null; onDone: () => void }) {
  ...
  // Max position notional = balance * leverage; size = notional / price.
  const maxSize = price > 0 ? (balance * leverage) / price : 0;
  const size = maxSize > 0 ? (maxSize * pct) / 100 : 0;
  const sizeStr = size > 0 ? String(Number(size.toFixed(6))) : "";
  const margin = price > 0 && size > 0 ? (size * price) / leverage : 0;

  async function submit(side: "LONG" | "SHORT") {
    if (size <= 0) { setNote("choose a size with the slider"); return; }
    setBusy(side); setNote("");
    try {
      const o = await api.futuresOrder({ symbol, side, size: sizeStr, leverage: lev });
      setNote(`${side} opened @ ${trimAmount(o.entry_price)} · margin ${trimAmount(o.margin)} USDT`);
      setPct(0); onDone();
  ...
  return (
    <div className="fut-form">
      ...
      <div className="of-field"><label>Size ({base})</label>
        <div className="of-input readonly"><input value={sizeStr} readOnly placeholder="0" /><span className="of-unit">{base}</span></div>
      </div>
      ...
      <div className="of-row"><span>Avbl</span><span className="mono">{balance.toFixed(2)} USDT</span></div>
      <div className="of-row"><span>Margin</span><span className="mono">{margin > 0 ? `${margin.toFixed(2)} USDT` : "—"}</span></div>
        ...
        {positions.map((p) => {
          ...
          return (
            <div className="fut-r" key={p.id}>
              ...
              <span className={`num mono ${pnl >= 0 ? "up" : "down"}`}>{pnl >= 0 ? "+" : ""}{pnl.toFixed(2)} <em>({Number(p.roe ?? 0).toFixed(1)}%)</em></span>
...
function TransferModal({ onClose, onDone }: { onClose: () => void; onDone: (a: FuturesAccount) => void }) {
  ...
  async function go() {
    ...
    try { onDone(await api.futuresTransfer(amount, deposit)); }
  ...
  return (
    <div className="mgt-modal" onClick={onClose}>
      <div className="mgt-box" onClick={(e) => e.stopPropagation()}>
        <div className="mgt-box-h"><h3>Transfer USDT</h3><button onClick={onClose}>✕</button></div>
        ...
        <div className="mgt-frow"><input value={amount} onChange={(e) => setAmount(e.target.value)} inputMode="decimal" placeholder="Amount USDT" /></div>
        {err && <p className="mgt-err">{err}</p>}
